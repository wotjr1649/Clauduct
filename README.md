...
기본은 **astra/medium**입니다. sol/xhigh, terra/high, luna/max를 지원합니다. 실행 중 `/model` 선택을 사용하며, 모델 미지정 Explore와 일반 역할은 luna/max, Plan은 sol/xhigh로 연결합니다. 자식 메타데이터와 부모 호출 ID로 확인한 명시 모델이 역할 기본값보다 우선합니다. 실제 실행에서 연결을 확인할 수 없으면 AGENT_SELECTION_UNVERIFIED로 중단하며 다른 모델로 추정 배정하지 않습니다.
...
- 메인·서브에이전트에 500K 컨텍스트/400K 자동 압축 정책을 전달합니다. 실제 backend 수용량과 실제 압축 시점은 별도 검증 대상입니다.
