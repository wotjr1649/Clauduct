...
export function requestStatusSnapshot(value, env = {}) {
  ...
  const projectRow = row => ({ request: number(row?.request),
    ...
    contentBlocks: row?.contentBlocks ? { text: counter(row.contentBlocks.text) ?? 0,
      toolUse: counter(row.contentBlocks.toolUse) ?? 0, thinking: counter(row.contentBlocks.thinking) ?? 0,
      serverToolUse: counter(row.contentBlocks.serverToolUse) ?? 0,
      searchResult: counter(row.contentBlocks.searchResult) ?? 0 } : null,
