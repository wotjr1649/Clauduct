...
| 항목 | 값 |
|---|---|
| 실행한 V2 Go 테스트 | **1124개 통과 / 0 실패 / 3 skip** (subtest 포함), 테스트가 있는 15 package. `CGO_ENABLED=0 go test ./... -count=1` 2026-09-17 실행 |
| 실행한 V2 Go 테스트 (0.3.0) | **18 package 전부 통과 / 0 실패**, race 18 package 통과, gofmt·vet exit 0. `CGO_ENABLED=0 go test ./... -count=1`과 `CGO_ENABLED=1 go test -race ./... -count=1`, 2026-09-21 실행. 위 행은 2026-09-17 시점의 기록이다 |
