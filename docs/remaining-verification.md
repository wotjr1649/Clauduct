...
현재 버전 조사 기준은 실제 세션에 기록된 Claude Code 2.1.266이다. 다음 세션에서 버전과 기능 목록을 재확인한다. 플러그인/내장 기능 전체 목록은 아직 수집 완료되지 않았으므로 이름을 추정해서 완성하지 않는다. 새 기능은 이 표 아래에 경로별 행을 추가하고 상세 기록에 연결한다.
