...
| 항목 | 값 |
|---|---|
| 실행한 V2 Go 테스트 | **1030개 통과 / 0 실패 / 3 skip** (subtest 포함), 14 package. 2026-09-17 실행 |
| mutation 검증 | **230건 주입** (battery 15개) + 2026-09-17 수동 4건. 현재 전부 잡힌다. 처음 주입 때 살아남은 것은 각 절에 기록했다 |
| 실모델 호출 | **추론 35회.** 1.3절(상한) · 1.4절(wire) · 1.6절(G7 실세션) · 1.9절(LIFE17) · 6.8절(A그룹) · D5(헤더) |
| 잔여 승인 예산 | **65회** (2026-09-15 사용자가 누적 100회로 상향) |
...
**관측한 버전을 계정에 적는다.** 클라이언트는 매 요청 `User-Agent: claude-cli/<버전>`을 보내므로
프로세스를 하나 더 띄우지 않고도 알 수 있다. 게이트웨이가 처음 한 번만 기록하고(세션에 클라이언트는
