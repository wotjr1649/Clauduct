...
**조건 1에 대한 사실 하나 (2026-09-17).** 사용자가 이 머신의 설치된 롤백 경로를 지웠다 —
`clauduct-node.cmd`와 `clauduct-node-store`, 그리고 Go 바이너리 백업 폴더. 되돌린 것이 아니라
**되돌릴 것을 치운 것**이고, 조건 1이 묻는 "되돌린 적이 없는가"는 여전히 예다. store에 박혀
있던 버전은 `4f3e375…`로 **태그 `v0.1.0`과 같은 커밋**이므로 필요해지면 그 릴리스로 똑같이
복구된다.

**이것이 정하지 않는 것: `src/`는 남는다.** CI의 Node job이 `node --test src/test-*.mjs`를 돌리고
PARITY가 그 소스를 기준선으로 인용한다. 설치본을 지운 것과 저장소에서 치우는 것은 다른 일이며,
후자는 여전히 조건 2와 3을 기다린다.
