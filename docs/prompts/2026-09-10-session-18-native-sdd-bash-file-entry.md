# Session-18: 보존된 RED에서 native SDD 계속
...
## 먼저: 검증된 Bash 파일 실행 경로

24a198f9-103f-4653-9cb1-89742850a80e는 session-17의 직접 PowerShell 도구 요구로 개발 전에 중단했다. 이후 현재 공유 guard의 명시적 pwsh -File 예외를 확인했고, 실제 Git Bash에서 기존 실행기·환경 검사·timeout 검사·전체 테스트와 개별 RED 실행을 검증했다. 별도 PowerShell 도구가 없어도 아래 파일 실행 경로를 사용할 수 있다. 이는 로컬 경로 검증이며 이번 native SDD 완주 증거는 아니다.

이 문서와 D:/AIDEV/Clauduct/src/run-node-tests.ps1을 읽은 뒤 native Bash에서 다음 명령을 그대로 실행한다. 환경을 지우거나 PATH를 바꾸지 않는다. PowerShell 실행기 내부가 Node 자식 환경을 제한한다. -Command/-EncodedCommand/-ExecutionPolicy, shell -c, winpty, 새로운 wrapper나 실행기 재작성은 필요 없으며 사용하지 않는다. 다른 실행기로 대체하거나 실제 guard 거부를 우회하지 않는다.
...
위 명령의 timeout은 도구에서 60초 이상, 실행기 자체는 기본 60초를 유지한다. 메인과 구현 자식 모두 이 정확한 진입점을 사용한다. 특정 RED 검증은 아래 명령이다.

```bash
pwsh -NoProfile -NonInteractive -File D:/AIDEV/Clauduct/src/run-node-tests.ps1 -Root D:/AIDEV/Clauduct/verification/dev-sandbox/run-02 -TestFiles test/untag.test.mjs
```

pwsh 미발견·실행기 자체 오류·guard/권한 거부가 실제 발생하면 코드 수정이나 구현 자식 생성 없이 SAFE-STOP하고 정확한 제한을 보고한다. powershell.exe로 대체하거나 설치·전역 설정을 변경하지 않는다. 직접 PowerShell 도구 부재만으로 중단하지 않으며, 과거 session-17의 금지 조건을 이 작업에 다시 적용하지 않는다. 상위 지침이나 실제 도구 거부는 그대로 따른다.
...
1. **S18-implementer**: 보존된 테스트와 기존 API를 읽는다. 같은 실행기에 -TestFiles test/untag.test.mjs를 지정해 5개 RED를 다시 관측한 뒤 구현한다. 기존 RED를 새로 썼다고 보고하지 않는다. 쓰기 파일은 tasks.mjs, cli.mjs, README.md, test/untag.test.mjs, test/boundaries.test.mjs뿐이다. 새 API의 경계 거부는 기존 격리 앱 테스트 방식을 재사용한다. 전체 실행기로 GREEN·회귀를 확인하고 명령·exit code·테스트 수·변경 파일을 반환한다. 커밋은 하지 않는다.
2. **S18-spec-reviewer**: 별도의 새 자식이 실제 diff와 테스트를 명세에 대조한다. 원래 untracked RED 파일은 Git diff만으로 보이지 않을 수 있으므로 파일도 읽는다. 구현자의 보고만으로 통과시키지 않는다. 수정·테스트 실행·커밋 없는 읽기 전용 검토다.
3. **S18-quality-reviewer**: 명세 검토가 완료·통과한 뒤 새 자식으로 오류 처리·데이터 보존·멱등성·공통 API·경계 검증·테스트 품질을 읽기 전용 검토한다. 결함은 위치와 근거를 반환한다.
