...
- A: 검증된 Agent/Task 호출과 metadata의 명시 모델을 먼저 적용한다. inherit는 native 요청 모델을 유지한다. 역할 기본값은 Explore/general-purpose=luna/max, Plan=sol/xhigh다. role=claude는 역할 강제 모델이 없어 요청 모델을 사용한다. 모든 자식의 컨텍스트 환경 정책은 500K/400K이며 backend 실제 수용량을 증명하지 않는다.
...
| Workflow | native Workflow → local inline run → workflow-subagent | 인증된 호출·run·nested metadata 연결. 명시 sidecar 모델은 요청과 대조. 최종 text는 reasoning 뒤로 전달 | 기본 상속 08594a7e·명시 luna/high 660a5d7d 실제 통과. 같은 run 혼합 자식 2개는 로컬 검사만 통과. 병렬·중첩·resume·custom agentType은 별도 범위. [혼합 시험 준비](audit-2026-09-09-workflow-mixed-children.md) |
