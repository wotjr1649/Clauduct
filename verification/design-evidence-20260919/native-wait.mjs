...
// Public fixtures, loopback transport, no generated shell/file tool calls or hooks.
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(leaf&&agentId===leaf){
      ...
      schedule(()=>{leafReleased=true;manifest.records.push({event:'leaf-release',agentId,ms:Date.now()-begun});save();emit(res,body.model,[text('LEAF_NATIVE_WAIT_OK')]);},12000);
      ...
      else{rec.kind='middle-woke';emit(res,body.model,[text(rec.hasLeaf?'MIDDLE_NATIVE_WAIT_OK':'MIDDLE_MISSING_LEAF')]);}
      save();
    }else{
...
child=spawn(native,['--session-id',id,'--model','gpt-6-astra','--effort','low','--strict-mcp-config'],{cwd:project,env,stdio:'inherit',windowsHide:true});
