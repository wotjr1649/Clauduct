...
func main() {
	for _, target := range []struct{ network, address string }{{"tcp4", "127.0.0.2:0"}, {"tcp4", "127.1.0.1:0"}} {
		for _, delay := range []time.Duration{0, 10 * time.Millisecond} {
			for attempt := 0; attempt < 3; attempt++ {
				...
				go func() {
					...
					done <- fmt.Sprintf("server_read=%d/%d server_read_error=%v request_equal=%t server_write=%d server_write_error=%v", n, len(request), readErr, string(input) == request, written, writeErr)
				}()
				...
				time.Sleep(delay)
				shutdownErr := c.(*net.TCPConn).CloseWrite()
