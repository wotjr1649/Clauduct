...
1. **n=6이고, 결측이 무작위가 아니다.** `src/native-gateway.mjs:367`이 `firstTextDeltaMs`를 `response.output_text.delta`에서만 기록하므로, **텍스트 없이 도구 호출로 끝난 턴에는 이 필드가 존재할 수 없다.** 실제로 34건 중 text 블록이 0인 18건은 전부 결측이고, text가 있는 8건 중 6건에 값이 있다. 즉 이 분해는 **텍스트로 끝난 턴만** 본 것이고, 도구 호출로 끝난 다수 턴의 추론 비중은 측정되지 않았다.
...
- effort A/B 실측은 범위·비용 합의 후에만 한다.
