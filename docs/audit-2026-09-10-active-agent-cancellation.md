...
등록 객체에 해당 등록의 활성 요청 controller Set을 둔다. 본문을 읽고 선택 단계에 진입한 요청만 연결하고 모든 종료 경로의 finally에서 제거한다. 등록 종료/교체 시 기존 선택 대기와 이 Set의 요청을 취소한다. 역할 충돌은 취소 전에 거부한다. 새 등록의 요청, 형제 등록, 메인 요청에는 취소를 전파하지 않는다.
...
본문 수신/메모리 admission 대기 중 종료·같은 ID 재등록의 순서 경쟁은 이번 Set의 연결 시점보다 앞이므로 이번 검증에 포함하지 않는다. 실제 native 중단 UI/정상 종료 hook 발행 순서, 원격 backend 계산 자체의 즉시 중단, 전체 취소 조합 및 symlink/junction 동적 검사는 미검증이다. 전송 취소를 이미 전송한 내용의 회수라고 설명하지 않는다.

기존 자동 인증 실행·symlink 거부를 유지했다. 새 사용자 시험은 아직 요청하지 않는다. 다음 로컬 점검은 요청이 특정 등록에 연결되기 전의 admission/본문 수신 구간이며, 실제 UI 시험은 그 차이를 정리한 뒤 설계한다.
