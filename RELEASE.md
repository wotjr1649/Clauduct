...
현재 후보의 무인 개발 출하 판정은 **HOLD**다. HTTP/TCP 안정성 문제와 동적 symlink/junction 실행 검사는 최신 사용자 지시에 따라 출하 판정에서 제외한다. 제품의 경로·신원 보호는 유지한다. Workflow 정상 재개와 일부 중첩 자식의 결과/실패 처리 공백은 남아 있으며, native의 직접 알림 방식만을 유일한 해결책으로 요구하지 않는다. 메인·일반 Agent·신규 Workflow의 지정 라우팅과 실제 두 파일 개발은 두 조합에서 통과했다. 로컬 ZIP 생성은 파일 구성·무결성 검사이며 전체 출하 검증의 통과를 뜻하지 않는다. 과거 HTTP/TCP 검사 실패와 실사용 제약은 아래 문서에 보존한다.
