...
기준 커밋은 src, poc, bin, docs, verification, README.md, clauduct.cmd, .gitignore를 포함한다. 기존 clauduct-check.txt와 %SystemDrive% 디렉터리는 포함하지 않으며 삭제하지 않는다. 원격 push는 수행하지 않는다.
