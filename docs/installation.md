...
## GitHub Release 공개 후 사용할 명령

다음 주소는 Release에 네 파일을 게시한 뒤 사용할 배포 경로다. 이번 로컬 개발에서는 push나 Release 게시를 수행하지 않았고 온라인 설치를 검증하지 않았다.
...
설치기는 PowerShell 5.1 문법을 목표로 작성했지만 이 머신의 Windows PowerShell 5.1 검사는 실행 정책의 `UnauthorizedAccess`로 시작 전에 차단됐다. 정책을 우회하지 않았으므로 5.1 실행 호환성은 미검증이다. 다른 머신, 온라인 다운로드, 실제 사용자 PATH 갱신도 검증 완료로 표시하지 않는다. 빌드 스크립트는 PowerShell 7이 필요하다.
