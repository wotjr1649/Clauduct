...
func (g *Gateway) continuationScope(scope delegationScope, id string, binding agentBinding) (delegationScope, bool) {
	...
	d.mu.Unlock()
	if !known || explicitResume || choice.parent == "" || choice.session != scope.session || binding.ID != id || binding.SessionID != scope.session || !roleMatches(choice.role, binding.Role, choice.custom) {
		return scope, false
	}
