...
이 문서를 남기는 주 목적은 5장이다. 열려 있던 관찰 항목 2건은 2026-09-15에 둘 다 닫혔다 — 하나는 재발했고(5-1), 하나는 우려가 성립하지 않는다는 증거가 나왔다(5-2). 1~4장은 그 2건이 무엇을 뜻하는지 읽을 수 있게 하는 최소 맥락이다.
...
`DotnetHttpProbe.cs:81`의 `CancelAfter`(수정 전 값 7000)는 모드 예산(`:211`)과 별개인 **중첩 가드**다. 성공 경로의 파서 프로세스 기동이 여기 걸리면 취소 주체가 바깥 `deadline`이 아니므로 `:294`의
...
`DotnetHttpProbe.cs`의 시간 상수는 전수로 다섯 개이며, 그중 무언가를 판정하는 것은 모드 예산뿐이다 — `:81` 가드(수정함), `:97` kill 이후 `WaitForExit(2000)`, `:250` `Timeout.InfiniteTimeSpan`(토큰에 위임), `:233` `ResponseDrainTimeout = TimeSpan.Zero`(지연 아님), `:211` 모드 예산.
...
### 5-1. flake 재발 — 관측했고, 걸린 가드를 넓혔다

**재발했다.** #14 머지 커밋의 main 런 `34914204688`이 실패했다. #8의 진단 덕분에 실패가 스스로 위치를 말했다.

| 관측 | 값 |
|---|---|
| 메시지 | `sse-fixed (category=TIMEOUT, 31486ms)` |
| 파일 블록 시작 → 실패 | `00:43:14.274` → `00:43:58.861` (44.6초) |
| 걸린 바운드 | `verification/DotnetHttpProbe.cs:211`의 `normal` 모드 예산 (당시 30000) |
| 분류 | **가드**. 3장이 명시한다 — `normal`은 어떤 어서션도 그 경과시간을 읽지 않는다 |

3장 표의 두 번째 행(`ETIMEDOUT` 31169ms)과 같은 급의 값이며, 같은 러너의 같은 원인이다. 새 지점이 아니라 **#9가 덜 올린 지점**이었다 — #9는 pwsh를 띄우는 다른 두 테스트를 120000으로 올리면서 모드 예산은 30000에 두었다. 같은 성격의 가드에 값이 둘이었다.

**조치**: 모드 예산 30000 → 120000. 관측된 최대 지연의 약 4배이고 형제 가드와 같은 값이다. 검증 어서션은 그대로다. 대가는 실제로 행이 걸린 케이스가 실패까지 2분을 쓰는 것이고, 워커 가드 240000은 그런 케이스 **하나**를 견딘다(`verification/test-dotnet-http-transport.mjs:54`).

**다음 재발 시 할 일**(그대로 유효): 우회하거나 재실행으로 넘기지 말고, 메시지의 `category=`와 경과시간, 그리고 해당 파일 블록의 시작~실패 타임스탬프 차이를 먼저 기록한다. 그 값이 어느 바운드에 걸렸는지 3장의 표와 대조하면 위치가 나온다. 새 지점이면 그 바운드가 가드인지 검증 대상인지부터 구분한다 — 검증 대상이면 넓히지 않는다.

**아직 증명되지 않은 것**: 120000이 충분하다는 것. 31486ms는 30000 예산에서 끊긴 값이라 **실제 지연이 얼마였는지는 모른다**(검열된 관측이다). 같은 지점이 120000에서 또 걸리면 그때는 값을 올리는 것이 아니라 케이스마다 node 파서를 새로 띄우는 구조를 본다.

### 5-2. `deadline-45s` 상한 7초 여유 — 닫는다. 이 경로에는 기동 지연이 낄 자리가 없다

`verification/test-dotnet-http-transport.mjs:217`:

```js
if (testCase.mode === 'deadline-45s') assert(elapsedMs >= 44000 && elapsedMs < 52000);
```

이 항목은 "기동 지연이 이 케이스 도중에 발생하면 어서션이 정당하게 실패할 수 있다"는 **우려**로 열려 있었다. 그 우려가 성립하지 않는다.

**기계적 근거 1 — 이 경로에 spawn이 없다.** `deadline-45s` 케이스는 `hang: true`다(`verification/test-dotnet-http-transport.mjs:129`). 서버가 응답하지 않으므로 `SendAsync`에서 데드라인이 발화하고, 예산 안의 유일한 프로세스 기동인 `RunProcess(Node, …)`는 HTTP 읽기가 끝난 **뒤**에 있어 **도달하지 않는다**. 러너를 느리게 만드는 요인 자체가 이 케이스를 지나지 않는다. 31486ms를 만든 `sse-fixed`는 반대로 성공 경로라 그 spawn을 지난다.

**기계적 근거 2 — CI 실측.** `:225`가 통과할 때마다 경과시간을 찍는다. 연속 5개 런:
...
예산 45000 대비 초과가 **2~17ms**다. 상한 여유 7000ms의 약 1/400이다. 같은 러너가 다른 케이스에서 31초를 흘리던 그 런들에서 나온 값이다.

**결정: 변경 없음.** 상한 52000은 "아예 타임아웃이 안 난 게 아니다"를 확인하는 검증 대상이고, 넓힐 이유가 없다. 이전에 적어둔 선택지 3개는 전부 폐기한다.

**재개 조건**: CI에서 `elapsedMs`가 45100을 넘으면 "이 경로에 spawn이 없다"는 전제가 깨진 것이다. 그때는 상한을 넓히지 말고 무엇이 그 시간을 썼는지부터 찾는다.
