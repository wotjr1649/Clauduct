...
| Workflow | native Workflow → local inline run → workflow-subagent | 인증된 호출·run·nested metadata 연결. 최종 text는 reasoning 뒤로 전달 | c18a7379의 sol/high·Read 실제 성공, 빈 result 문제 수정 후 실제 복귀 미검증. 중첩·resume·custom agentType은 범위 밖. [분석·수정](audit-2026-09-09-workflow-result.md) |
