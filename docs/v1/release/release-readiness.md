...
4. [x] 릴리즈 구성의 전체 회귀, 배포 산출물 내용·경로·시작/종료 검사와 최종 판정을 기록했다. 실행하지 못한 symlink 검사는 성공으로 세지 않는다.
...
추가 러너 검사의 첫 실행은 배포본에 포함하지 않은 개발용 `README.md`를 전제로 해서 실패했다. 런타임의 경로 검사는 이를 허용하지 않고 `TEST_FILE_NOT_FOUND`로 거부한 상태였다. 존재하는 `.ps1` 거부를 검사하도록 fixture를 `src/test-run-node-tests.ps1`로 바꿨다. 소스 checkout과 아래 최종 배포본 모두에서 환경 격리 3/3·경로 거부 3/3·없는 루트·실패 exit·timeout 후 프로세스 트리 종료가 통과했다.

### 최종 빌드 — e216ad8

| 항목 | 관측 |
|---|---|
| 소스 commit | `e216ad8837285997e361317d7f25b76c49d1b5a0` |
| 산출물 | `Clauduct-e216ad883728.zip`, 296439 bytes, 파일 68개, 비압축 합계 1021692 bytes |
| ZIP SHA256 | `191a6654e92aee3785bda7a31df2553d381784951dedaa479e4aabfaa8e553b8` |
| 재현성 | 같은 commit의 독립 빌드 2회 SHA256 일치 |
| 설치 검증 | 새 공백 경로에서 68개 파일 크기·해시 일치, 실제 `clauduct.cmd --dry-run -p`와 `--help` exit 0 |
| 빌더 경계 | 배포본 디렉터리를 checkout으로 오인하지 않고 `RELEASE_NOT_CHECKOUT`, exit 1로 거부 |
| 배포본 추가 검사 | 러너 자기 검사 전부 통과, `verification/test-manual-http-probe.mjs` 88/88 |
| 최종 실제 왕복 | 새 프로필의 `stream-json`: partial text 정확, result 1개, 실제 luna/low, 모든 요청 성공, cleanup 9개 true, exit 0 |
| 앞선 배포본과 동일성 | 68개 중 66개 SHA256 동일. 변경된 두 파일은 러너 자기 검사와 릴리즈 빌더뿐이며 제품 런타임은 동일 |
| 원래 루트 반영 | `D:/AIDEV/Clauduct`, `fix/native-completion-resume`을 `6c263c0`에서 fast-forward. 그 위치의 전체 `src` 회귀 23/23, exit 0, 18.15초 |
| 보존·내용 검토 | 원래 untracked 목록 유지, 사용자 파일 stage 없음. ZIP의 제외 대상 상태 파일 0개, 비밀 키/개인키/JWT 고신뢰 패턴 후보 0개. 패턴 검사가 모든 종류의 비밀 부재를 수학적으로 증명하지는 않음 |
| 문서 검사 | 현행 문서 7개 상대 링크 72개 모두 존재, `git diff --check` 통과 |

최종 복합 명령은 `shell-guard`가 PowerShell 호출 연산자 `&` 때문에 실행 전에 거부했다. 거부된 복합 명령을 반복하거나 guard를 변경하지 않았다. guard가 안내한 명시적 명령 이름 형식으로 기존 빌더·검사 스크립트를 호출했고, 같은 guard가 허용한 위 결과를 따로 확인했다. 동적 symlink/junction 거부는 여전히 별개 차단 항목이다.

최종 검증 기록은 산출물 생성 뒤 문서만 추가한 것이다. 배포 ZIP은 위 commit에 고정되며 사용자 프로필이나 뒤의 검증 기록을 포함하지 않는다. SHA256 manifest는 ZIP과 같은 작업 전용 `release-artifacts-*` 디렉터리에 있다.
