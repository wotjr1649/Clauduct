...
않으며, 재기동해도 기존 예약은 남는다. 검증 모드의 검색은 기본적으로 `ROUTE_NOT_AUTHORISED`로 거부한다.
v0.4.4부터 계획에 `"allowSearch":true`를 명시하면 WebSearch의 최초 시도와 재시도도 같은 총상한에
예약한다. 인증 갱신을 위한 credential 재조회보다 먼저 예약하며, 검색은 `inferences`나 모델별 횟수에는
더하지 않는다. 검색 예약 파일의 `Search:true`와 실제 전송 통계 `searchTransport`를 함께 대조한다.
예약 후 인증·연결 등이 실패해 실제 전송하지 못해도 예약은 유지된다. 생략·false는 검색 거부이며,
null·boolean 이외 값·중복 키는 잘못된 계획이다. 형식 버전은 1을 유지한다. v0.4.3은 `allowSearch`가
있는 계획을 알 수 없는 필드로 거부하므로 검색 검증에는 v0.4.4 이상을 사용한다.
