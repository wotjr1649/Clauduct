...
Claude Code 2.1.282의 공개 옵션 형태(2.1.281과 같고, `--agents`가 `--print`와 함께 JSON 파일 경로도 받는다. 역할 탐색은 그 파일을 설정 파일처럼 제한해 읽는다)와 기존 hidden 옵션 목록을 사용하며 native 전체 구문을
...
활성 요청·연결·대기열·개별 frame·총 응답·로그·종료 대기에 명시적 상한을 둔다. goroutine이 가볍다는 이유로 무제한 작업을 만들지 않는다. 각 자원에 생성자·취소 원인·해제 조건·테스트가 있어야 한다. G4 이전에 limit registry의 단위·값·초과 동작·테스트를 모두 확정하며, 필수 제한을 `unknown`이나 무제한으로 둔 채 제품 후보로 승격하지 않는다. v0.4.2는 본문 읽기 전에 물리 메모리 기반 예약 예산과 현재 여유 메모리를 검사한다(#119). 생성·계수는 동시 실행 64건 상한과 유한 대기열을 쓰며, hook·이벤트는 별도 용량·대기열을 쓴다. 예약은 handler가 끝날 때 반환한다. 이 추정은 실제 RSS 상한이나 프로세스 간 합산 제한이 아니다. 수치·거부·한계는 [v0.4.2 정책](COMPATIBILITY.md#v042--메모리-수용-제어)이 소유한다.
