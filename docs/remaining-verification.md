...
| 모델 선택 | 역할 고정값 유지. 8884c607에서 네 GPT 시험 정의 실제 반환 확인, 비기본 부모 effort 상속 실패 재현 후 definition-inherit 연결 로컬 수정·검증 | [최신 계약과 증거](gpt-agent-selection-contract.md). 수정 버전의 실제 상속·기존 역할 보존 확인 후 일반 개발용 직접 선택 인터페이스를 정리. Claude 별칭 대체 검증은 제외. UI 배너를 backend 증거로 사용하지 않음 |
