...
export function createDevelopmentFixture({ waitMode = 'wait', holdAfterPass = false, holdAfterSourceWrite = false, taskId = DEFAULT_DEVELOPMENT_TASK_ID } = {}) {
  if (!['wait', 'check'].includes(waitMode) || typeof holdAfterPass !== 'boolean' || typeof holdAfterSourceWrite !== 'boolean'
    || holdAfterPass && holdAfterSourceWrite) throw new Error('INVALID_DEVELOPMENT_MODE');
  ...
  const args = ['--permission', '--allow-child-process', `--allow-fs-read=${script}`, `--allow-fs-read=${policy}`,
    ...
    ...(holdAfterPass ? ['hold-after-pass'] : holdAfterSourceWrite ? ['hold-after-source'] : [])];
