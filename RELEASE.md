...
이 머신의 사용 범위는 native 전역 `settings.json`의 `permissions.defaultMode=bypassPermissions`다. Clauduct는 native 설정 소스를 유지하며 이 모드를 별도로 덮어쓰지 않는다. 다른 permission 모드나 전환 조합은 이번 출하 조건에 포함하지 않는다. 검증에서는 확인한 모드 값만 공개 격리 fixture에 적용하고 개인 설정 원문은 복제하지 않는다.

이전 worker가 종료된 뒤의 저장 Workflow 재개는 두 모델에서 확인했다. worker가 살아 있는 상태의 중복 재개 음성 시험은 native2.1.270의 반환 scriptPath 읽기 검사에서 먼저 거부돼 생존 worker 판정 경계까지 확인하지 못했다. 원래 worker는 TaskStop으로 취소했고 중복 자식·잔여 프로세스는0이었다. 해당 native 거부 이력은 정상 재개 성공과 구분해 유지한다.
